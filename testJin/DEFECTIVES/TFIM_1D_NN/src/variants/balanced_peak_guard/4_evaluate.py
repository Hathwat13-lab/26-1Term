...
DATA = ROOT / "data"
MODELS = ROOT / "models" / "balanced_peak_guard"
FIGURES = ROOT / "figures" / "balanced_peak_guard"
...
def main():
    ...
    with (ROOT / "results_summary_balanced_peak_guard.md").open("w", encoding="utf-8") as f:
    ...
    for k, v in metrics.items():
        print(f"{k}: {v:.8e}")
    print(f"요약 저장: {ROOT / 'results_summary_balanced_peak_guard.md'}")
