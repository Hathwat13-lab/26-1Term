...
def critical_features(x):
    """Smooth features that make the h=1 critical fan easy to represent."""
    T = x[..., 0]
    h = x[..., 1]
    inv_L = x[..., 2]
    dh = h - CRITICAL_H
    thermal_rounding = 0.75 * T + 1.5 * inv_L + 0.03
...
def init_mlp(key, widths=(9, 96, 96, 96, 1)):
    params = []
