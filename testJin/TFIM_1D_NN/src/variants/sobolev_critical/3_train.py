...
DATA = ROOT / "data"
MODELS = ROOT / "models" / "sobolev_critical"
FIGURES = ROOT / "figures" / "sobolev_critical"
...
def _critical_weight(x):
    ...
    h = x[:, 1]
    inv_L = x[:, 2]
    h_focus = jnp.exp(-((h - 1.0) / 0.20) ** 2)
    low_t_focus = 1.0 / (1.0 + 4.0 * T)
    large_l_focus = 1.0 / (1.0 + 8.0 * inv_L)
    w = 1.0 + 10.0 * h_focus * low_t_focus + 3.0 * h_focus * large_l_focus
    return w / jnp.mean(w)


def _loss(params, xb, yb, xd, yd, chi_weight, cv_weight):
    ...
    return f_loss + chi_weight * chi_loss + cv_weight * cv_loss, (f_loss, chi_loss, cv_loss)


@jax.jit
def _train_step(params, opt_state, xb, yb, xd, yd, chi_weight, cv_weight):
    (loss, parts), grads = jax.value_and_grad(_loss, has_aux=True)(
        params, xb, yb, xd, yd, chi_weight, cv_weight
    )
...
def main():
    ...
    for epoch in range(1, epochs + 1):
        ...
        idx = _sample_mixed(kb, n, critical_idx, batch_size, critical_fraction=0.25)
        didx = _sample_mixed(kd, n, critical_idx, deriv_batch_size, critical_fraction=0.70)
    ...
    metadata = {
        "variant": "sobolev_critical",
        "story": "Critical features, critical-region derivative sampling, strong chi Sobolev loss, light Cv loss.",
        "train_L": [6, 8, 10, 12],
        "holdout_L": [14, 16],
        "epochs": epochs,
        "chi_weight": chi_weight,
        "cv_weight": cv_weight,
        "critical_sampling": {"batch_fraction": 0.25, "derivative_fraction": 0.70},
    ...
    np.savetxt(
        ...
        arr,
        delimiter=",",
        header="epoch,train_F_mse,holdout_F_mse,train_chi_mae,holdout_chi_mae,total_batch_loss,batch_F_loss,batch_chi_loss,batch_Cv_loss",
        comments="",
    )
