...
* **C. 물질 역설계 (Inverse Design)**: 타겟 상전이 특성(Target Phase Transition)을 가지는 물질의 파라미터를 역추적하는 설계 진행. 예시: 차단 온도가 높은 나노마그넷(자기 이방성 파라미터도 추가로 고려)의 역설계 등
* **D. 나노마그넷 표면 효과 탐구**: GNN을 활용한 나노마그넷의 표면 효과 탐구
* **E. e3nn 기반 대리모델**: 3D 분자 구조 입력으로 J, D, F 중 1개를 예측하는 e3nn 기반 대리모델
